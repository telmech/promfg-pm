  ...
  if (fs.existsSync(schemaPath)) {
    ...
    db.transaction(() => {
      ...
      `).run(
        ...
        'active',
        new Date().toISOString(),
        JSON.stringify({ projects: true, bom: true, rfq: false, pr: true })
      );
      ...
      db.prepare(`
        INSERT INTO settings (org_id, company_name, company_logo)
        VALUES (?, ?, ?)
      `).run(ORG_ID, 'PRO-MFG', 'logo.jpg');
